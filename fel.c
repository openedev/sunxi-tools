...
void aw_fel_write_uboot_image(feldev_handle *dev, uint8_t *buf, size_t len)
{
	...
	memcpy(hdr, buf32, HEADER_SIZE);

	uint32_t hcrc = be32toh(hdr[1]);
	hdr[1] = 0;
	...
	uint32_t dcrc = be32toh(buf32[6]);
	uint32_t computed_dcrc = crc32(0,
				       ((const uint8_t *) buf32) + HEADER_SIZE,
				       data_size);
